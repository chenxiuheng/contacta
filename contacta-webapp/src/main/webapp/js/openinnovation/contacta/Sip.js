...
{
  return declare("openinnovation.contacta.Sip", [ Controller ],
  {
    classCode:"sip",
    ...
    convertJson2Form:function(json)
    {
      ...
      if (p.id === 0)
      {
        this._ui.loginInput.set('readOnly', false);
      }
      else
      {
        //this._ui.loginInput.set('readOnly', true);
      }
    ...
    paging:function(searchValue)
    {
      if(searchValue)
      {
        ui.sip.filterInput.set('value',searchValue);
      }
      else
      {
    ...
    changeProfile:function(arg)
    {
      ...
      if (arg === 'Segretaria')
      {
        ui.profileOptions.set('required', true);
        ui.profileOptions.set('regExp', '\\d{4},\\d{4}');
        ui.profileOptions.set('promptMessage', 'Interni del direttore controllati: privato,pubblico.  <br/>Il numero privato deve esistere');
        ui.profileOptions.set('invalidMessage', 'Inserire gli interni XXXX,YYYY<br/>rispettivamente privato e pubblico.<br/>Il numero privato deve esistere');
      }
      else if (arg === 'Dial a cascata')
      {
        ui.profileOptions.set('required', false);
        ui.profileOptions.set('regExp', '\\d{4}(,\\d{4})*');
        ui.profileOptions.set('promptMessage', 'Lista di interni');
        ui.profileOptions.set('invalidMessage', 'Lista di interni a quattro cifre separati da virgola');
      }
      else
      {
        ui.profileOptions.set('required', false);
        ui.profileOptions.set('regExp', '.*');
        ui.profileOptions.set('promptMessage', 'Questo campo viene ignorato');
        ui.profileOptions.set('invalidMessage', null);
      }
